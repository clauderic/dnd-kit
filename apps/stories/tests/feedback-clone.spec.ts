...
import type {} from './fixtures/feedback-clone.ts';

let fixture: string;
test.beforeAll(async () => {
  // Bundle the fixture for both static CI hosting and the development server.
  const result = await build({
    ...
    build: {
      ...
      lib: {
        entry: fileURLToPath(
          new URL('./fixtures/feedback-clone.ts', import.meta.url)
        ),
        name: 'feedbackFixture',
        formats: ['iife'],
      },
    },
  });
    ...
    const result = await page.evaluate(async (change) => {
      ...
      try {
        ...
        if (change === 'descendant content') {
          ...
          const target = new Droppable({id: 'child', element: child}, manager);
          target.register();
          targets.push(target);
        }
        await frame();
