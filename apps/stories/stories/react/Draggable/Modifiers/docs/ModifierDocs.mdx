...
| `SnapToPointer` | Offsets the drag transform so the element's center snaps to the cursor position |

#### `SnapToPointer`

When dragging starts, `SnapToPointer` immediately repositions the element so the specified anchor point aligns with the cursor. Defaults to `center`. Other options are `top-left`, `top-right`, `bottom-left`, and `bottom-right`.
