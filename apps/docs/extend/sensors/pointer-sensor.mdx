...
<ParamField path="Distance" type="class">
  ...
  **Options:**
  - `value` (number): Required distance in pixels.
  - `tolerance` (number | {x?: number; y?: number}, optional): If specified, aborts activation if movement exceeds this threshold. Can be a number or an object with optional x and y properties.
</ParamField>
...
<ParamField path="Delay" type="class">
  ...
  **Options:**
  ...
  - `tolerance` (number | {x?: number; y?: number}): Maximum movement allowed during delay. Can be a number or an object with optional x and y properties. Exceeding this aborts activation.
</ParamField>
