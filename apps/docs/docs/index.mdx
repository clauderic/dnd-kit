---
title: Overview
description: 'Learn how to build drag and drop interfaces with <span class="inline-logo">**dnd kit**</span>'
hideContextMenu: true
ogTitle: "The modern toolkit for building\ndrag & drop interfaces"
ogDescription: ''
seo:
  title: 'dnd kit – The modern toolkit for building drag and drop interfaces'
  description: 'An extensible drag and drop toolkit written in TypeScript. Framework-agnostic core with first-class integrations for React, Vue, Svelte, Solid, and Marko.'
---

{/* Hero SVG is inlined by the page template for faster LCP */}

## Key features
- **Framework agnostic**: First-class support for React, Vue, Svelte, Solid, Marko, and TypeScript
- **Batteries included**: Drag, drop, sort, and reorder in any layout or direction
- **Fully extensible**: Plugins, sensors, and modifiers for complete control
- **Production ready**: Built for performance, accessibility, and reliability

## Getting started

Choose your preferred framework to get started:

<CardGroup cols={2}>
  <Card
    title="TypeScript"
    icon="ts"
    href="/quickstart"
  >
    Build drag and drop interfaces using plain TypeScript
  </Card>
  <Card
    title="React"
    icon="react"
    href="react/quickstart"
  >
    Build drag and drop interfaces using React components and hooks
  </Card>
  <Card
    title="Vue"
    icon="vuejs"
    href="vue/quickstart"
  >
    Build drag and drop interfaces using Vue composables and components
  </Card>
  <Card
    title="Svelte"
    icon="svelte"
    href="svelte/quickstart"
  >
    Build drag and drop interfaces using Svelte primitives and components
  </Card>
  <Card
    title="Solid"
    icon="solidjs"
    href="solid/quickstart"
  >
    Build drag and drop interfaces using SolidJS hooks and components
  </Card>
  <Card
    title="Marko"
    icon="code"
    href="marko/quickstart"
  >
    Build drag and drop interfaces using Marko v6 tags and components
  </Card>
</CardGroup>

## Core Concepts

Learn the essential building blocks:

<CardGroup cols={2}>
  <Card
    title="Manager"
    icon="sitemap"
    href="concepts/drag-drop-manager"
  >
    The central orchestrator that coordinates drag and drop operations
  </Card>
  <Card
    title="Draggable"
    icon="bullseye-pointer"
    href="concepts/draggable"
  >
    Elements that can be picked up and dragged to new locations
  </Card>
  <Card
    title="Droppable"
    icon="expand"
    href="concepts/droppable"
  >
    Target areas where draggable elements can be dropped
  </Card>
  <Card
    title="Sortable"
    icon="layer-group"
    href="concepts/sortable"
  >
    Draggable elements that can be reordered within and across lists
  </Card>
</CardGroup>

## Extend and customize

Make it your own with powerful extension points:

<CardGroup cols={2}>
  <Card
    title="Plugins"
    icon="cube"
    href="extend/plugins"
  >
    Add new features and behaviors with the plugin system
  </Card>
  <Card
    title="Sensors"
    icon="signal-stream"
    href="extend/sensors"
  >
    Support different input methods like mouse, touch, and keyboard
  </Card>
  <Card
    title="Modifiers"
    icon="sliders"
    href="extend/modifiers"
  >
    Modify drag behavior with constraints and transformations
  </Card>
</CardGroup>



## Sponsors

<div className="sponsors">
  <Frame>
    <a href="https://sentry.io/" target="_blank" aria-label="Sentry">
      <img className="dark:hidden" src="/images/sponsors/sentry-logo.svg" alt="Sentry" style={{width: 180, height: 80, pointerEvents: "none"}} />
      <img className="hidden dark:block" src="/images/sponsors/sentry-logo-dark.svg" alt="Sentry" style={{width: 180, height: 80, pointerEvents: "none"}} />
    </a>
  </Frame>
  <Frame>
    <a href="https://www.any.do/" target="_blank" aria-label="Any.do">
      <img className="dark:hidden" src="/images/sponsors/any-do-logo.svg" alt="Any.do" style={{width: 120, height: 80, pointerEvents: "none"}} />
      <img className="hidden dark:block" src="/images/sponsors/any-do-logo-dark.svg" alt="Any.do" style={{width: 120, height: 80, pointerEvents: "none"}} />
    </a>
  </Frame>
  <Frame>
    <a href="https://doist.com/" target="_blank" aria-label="Doist">
      <img className="dark:hidden" src="/images/sponsors/doist-logo.svg" alt="Doist" style={{width: 90, height: 80, pointerEvents: "none"}} />
      <img className="hidden dark:block" src="/images/sponsors/doist-logo-dark.svg" alt="Doist" style={{width: 90, height: 80, pointerEvents: "none"}} />
    </a>
  </Frame>
  <Frame>
    <a href="https://puckeditor.com/" target="_blank" aria-label="Puck" style={{position: "relative", top: 4}}>
      <img className="dark:hidden" src="/images/sponsors/puck-logo.svg" alt="Puck" style={{width: 130, height: 80, pointerEvents: "none"}} />
      <img className="hidden dark:block" src="/images/sponsors/puck-logo-dark.svg" alt="Puck" style={{width: 130, height: 80, pointerEvents: "none"}} />
    </a>
  </Frame>
  <Frame>
    <a href="https://httpie.io/" target="_blank" aria-label="HTTPie">
      <img className="dark:hidden" src="/images/sponsors/httpie-logo.svg" alt="HTTPie" style={{width: 120, height: 80, pointerEvents: "none"}} />
      <img className="hidden dark:block" src="/images/sponsors/httpie-logo-dark.svg" alt="HTTPie" style={{width: 120, height: 80, pointerEvents: "none"}} />
    </a>
  </Frame>
  <Frame>
    <a href="https://mintlify.com/" target="_blank" aria-label="Mintlify">
      <img className="dark:hidden" src="/images/sponsors/mintlify-logo.svg" alt="Mintlify" style={{width: 140, height: 80, pointerEvents: "none"}} />
      <img className="hidden dark:block" src="/images/sponsors/mintlify-logo-dark.svg" alt="Mintlify" style={{width: 140, height: 80, pointerEvents: "none"}} />
    </a>
  </Frame>
</div>
