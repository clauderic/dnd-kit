...
<AccordionGroup>
  <Accordion title="Default configuration">
    ...
    **Plugins**
    ...
    - `ScrollCompensator`: Tracks scroll-induced displacement during drag
...
- `dragOperation`: Current drag operation state
  ...
  - `transform`: Cumulative pointer displacement after modifiers pipeline
  - `scrollAdjustment`: Cumulative scroll delta
  - `adjustedTransform`: Cumulative source displacement in document coordinates (`transform` + `scrollAdjustment`)
