...
<Story id="droppable-basic-setup--example" framework="marko" height="400" hero />
...
<ResponseField name="setElement" type="(el: Element | undefined) => void">
  Connect a DOM element to this droppable. Call in `onMount` with `$el()`, and in `onDestroy` with `undefined`.
</ResponseField>
