...
<Story
  id="sortable-vertical-list--basic-setup"
  framework="marko"
  height="320"
  hero
/>
...
<Info>
  Learn more about [type guards and manual state
  management](/concepts/sortable#optimistic-sorting) in the Sortable concepts
  page.
</Info>
...
<ParamField path="collisionPriority" type="number" optional>
  The collision priority of this sortable element. Higher values take precedence
  when multiple droppable elements overlap.
</ParamField>
...
<ResponseField name="setElement" type="(el: Element | undefined) => void">
  Connect a DOM element to this sortable. Call in `onMount` with `$el()`, and in
  `onDestroy` with `undefined`.
</ResponseField>

<ResponseField name="setHandle" type="(el: Element | undefined) => void">
  Set a drag handle element. Call in `onMount` with `$el()` on the handle
  element.
</ResponseField>
