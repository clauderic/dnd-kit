...
A sortable element is an element that is simultaneously [draggable](/concepts/draggable) and [droppable](/concepts/droppable).

<Info>
  The `useSortable` hook is a thin wrapper around the [Sortable](/concepts/sortable) class that makes it easier to create sortable elements in React.
</Info>
