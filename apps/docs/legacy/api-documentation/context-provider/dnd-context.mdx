...
Fires anytime as the [draggable](../draggable/overview) item is moved. Depending on the activated [sensor](../sensors/#activators), this could for example be as the [Pointer](../sensors/pointer) is moved or the [Keyboard](../sensors/keyboard) movement keys are pressed.

#### `onDragOver`

Fires when a [draggable](../draggable/overview) item is moved over a [droppable](../droppable/overview) container, along with the unique identifier of that droppable container.
...
<Info>
It's important to understand that the `onDragEnd` event **does not move** [**draggable**](../draggable/overview) **items into** [**droppable**](../droppable/overview) **containers.**
