...
- 13 collision browser regressions, 30 nested-collection story cases, and 3 clone-feedback lifecycle cases passed.
...
The nested-collection story at `React/Sortable/Nested collections/Example` is a finished board with recursive collections, card transfers, empty groups, root reordering, keyboard reversal, cancellation, and reset. Cards and collections accept each other as reorder targets and can both enter collection contents. They retain the normal collision priorities, preserving pointer containment ahead of shape-only intersections. A collection background transfers children from other groups into the slot at the pointer and preserves the order of children already in that group. Collection reorder targets use their header rectangles, and only the labeled root band appends at root; the surrounding board is layout, not an append target. Its tree update guards against cycles. All eight diagnostic stories remain available. Light, dark, and 390px layouts were visually inspected.

The polished story also has an optional 500ms hover delay before pointer/touch transfers to a different parent. A destination highlight previews the move without changing layout. Moving between targets in that destination preserves the countdown; leaving, releasing, canceling, or unmounting clears it. On expiry, the story requests a fresh collision pass and lets the usual target action apply the latest destination. Reordering within the current parent and keyboard commands remain immediate. This is a story presentation choice, adjustable through its Container hover delay control; the library still adds no timer. The existing placement-path and responsiveness regressions explicitly use a zero delay, while fifteen browser cases check the countdown, insertion slots, and cancellation with a controlled clock.
...
The first local browser checks used the development server and a taller viewport than the default CI configuration. Static CI exposed a development-only `/@fs` import in the clone fixture and nested gestures that pointed below the viewport. The fixture now bundles its public and private modules together before browser injection, and nested tests declare their 1440×1100 gesture viewport. Recorded paths retain their captured viewport and the narrow-screen case still overrides it.

Four integration test files imported the downstream collision package from abstract tests, racing its build on a clean checkout. They now run in the collision package, which declares its test command and state test dependency. The abstract tests no longer require collision to be built. Turbo output patterns now describe paths relative to each package, including root-level entry points and DOM plugin outputs. Removing abstract's twelve generated entries and rerunning from cache restores all twelve and passes its tests.

All 274 package tests pass through the normal task graph in a separate checkout (47 abstract, 60 collision, 100 DOM, 7 state, and 60 helpers). The 207-test collision-focused subset is unchanged. All 81 React browser tests pass against the built Storybook and Playwright's bundled Chromium with two workers and retries disabled, rather than relying on the locally installed Chrome or the running development server.


## Cross-container insertion follow-up

Hovering five pixels above the first card or beside it selected the enclosing contents target. The story treated every contents target as append, so the card went to index two instead of zero after the hover delay. Direct card targets already chose before or after the card correctly.

Container transfers now find the first immediate child whose measured target midpoint is below the current pointer. This uses the collision registry's existing geometry, including collection header targets, without additional DOM measurements. Positions below all children still append; the root append band, own-parent background behavior, and keyboard placement semantics are unchanged. The delay continues to resolve the latest slot rather than capturing an index when the countdown starts.

Eight insertion regressions cover top padding, side padding, both halves of a card, the gap before a collection, nested top padding and gaps, and the append region. Each checks the exact sibling order after transfer, a stationary interval, and drop. The existing countdown-progress case now also checks that moving from the append band to the first card inserts first without restarting the delay. The baseline top-padding case failed with the card at the end.

The three recorded paths now produce six, eight, and zero placements. Container transfers use their hovered indices; the second path no longer appends to Ideas before immediately moving to its first slot. These tests retain exact placement-sequence and connected-geometry assertions.

All 89 React browser tests pass against the built Storybook with Chromium 147, two workers, and retries disabled. Targeted TypeScript and formatting checks also pass.
