...
<Callout style="warning">
  ...
  it is a descendant of a `DndContext` provider. You may nest multiple
  `SortableContext` components within the same parent `DndContext`.
</Callout>
...
<Callout>
  In most cases, the draggable and droppable hooks will be attached to the same
  node, and therefore be identical in size. They are represented as different
  nodes for illustration purposes above.
</Callout>

If you're already familiar with the [`useDraggable`](../../api-documentation/draggable/) hook, the [`useSortable`](usesortable) hook should look very familiar, since, it is an abstraction on top of it.
...
For more details on the `useSortable` hook, read the full [API documentation](usesortable).
